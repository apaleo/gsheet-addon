...
function generateCityTaxReport(city, property, startDate, endDate) {
    ...
    const revenueTransactions = city === CityTax.BERLIN 
        ? getRevenueTransactions(property,startDate, endDate)
        : [];
    var reservations = APIData.getReservations(property, DateUtility.addDays(startDate, -1), DateUtility.addDays(endDate, 1));
    var sheet = createSheetWithReportInfo(city, property, endDate, startDate);
    switch (city) {
        case CityTax.BERLIN:
            generateBerlinCityTax(sheet, transactions, reservations, revenueTransactions,cityTaxConfigs);
            break;
...
function generateBerlinCityTax(sheet, transactions, reservations, revenueTransactions, cityTaxConfigs) {
    var rows = [];
    ...
    const revenueByChannel = _(revenueTransactions)
        .map(item => {
            const reservation = _.find(reservations, function(reservation) { return reservation.id == item.reference || reservation.bookingId == item.reference; });
    ...
    var channelKeysFromCityTax = _(transactionsWithReservations)
     .map(function (v) { var _a; return (_a = v.source) !== null && _a !== void 0 ? _a : v.channelCode; })
     .filter(Boolean)
     .uniq()
     .value();
   var channelKeysFromRevenue = _.keys(revenueByChannel);
   var allChannelKeys = _.uniq(_.concat(channelKeysFromCityTax, channelKeysFromRevenue));

   var summarizedData = _.map(allChannelKeys, function (key) {
     var value = _.filter(transactionsWithReservations, function (v) {
       var _a; return ((_a = v.source) !== null && _a !== void 0 ? _a : v.channelCode) === key;
     });
    ...
    var includedRates = _.uniq(_.map(cityTaxConfigs, function (c) {
        var type = c.vatType || "Without";
        return vatLabels[type] || type;
    }));

    var noteLines = [
        "Net accommodation revenue includes only room-revenue postings taxed at the same VAT rate(s) as this property's configured city tax: " + includedRates.join(", ") + ".",
...
function getTransactions(property, transactionStartDate, transactionEndDate, startDate, endDate) {
    ...
    for (var _i = 0, cityTaxConfigs_1 = cityTaxConfigs; _i < cityTaxConfigs_1.length; _i++) {
        ...
        var accountNumber = "CityTax_".concat(vatType, ":").concat(percentage.toFixed(2));
        // Get transactions for this account
        var transactions = APIData.getAccountTransactions(property, accountNumber, transactionStartDate, transactionEndDate);
    ...
    return allTransactions.filter(function (transaction) { return transaction.command == "PostCharge" && reportDaysList.includes(transaction.date); });
}
...
function getRevenueTransactions(property, startDate, endDate) {
    ...
    var reportDaysList = DateUtility.getDates(startDate, endDate)
        .map(function(d) { return d.toISOString().slice(0, 10); });
    if (reportDaysList.indexOf(endDate) === -1) reportDaysList.push(endDate);
    ...
    for (var _i = 0; _i < cityTaxConfigs.length; _i++) {
        ...
        var percentage = 0;
        switch (vatType) {
            case "Reduced": percentage = 7;  break;
            case "Normal":  percentage = 19; break;
        ...
        var accountPattern = new RegExp(
            "^RevenueAccommodation_([A-Za-z0-9]+-)?[A-Za-z]+:" + percentage.toFixed(2) + "$"
        );

        var filtered = transactions.filter(function(t) {
            return t.command === "PostCharge" &&
