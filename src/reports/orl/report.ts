...
// ---------------------------------------------------------------------------
// Tune this down (e.g. 3) if weekly windows still overflow the 50MB response
 ...
 * months. This keeps each individual API response small and predictable,
 * regardless of how many days are in a given month. If any single chunk
 * fails (e.g. still too large), the error propagates and the whole report
 * fails — no retry/split logic.
 *
...
function fetchGrossTransactionsChunked(property, startDate, endDate) {
  ...
  while (cursor.isSameOrBefore(finalEnd, "day")) {
    ...
    data.push.apply(
      data,
      APIData.getGrossTransactions(property, chunkStartStr, chunkEndStr),
    );
...
function generateORLReport(
  ...
  // @ts-ignore
  var data = fetchGrossTransactionsChunked(property, startDate, endDate);
  ...
  var intialState = {};
  var groupedRecords = Object.values(
    transactions.reduce(function (groups, transaction) {
  ...
  for (
    var _i = 0, groupedRecords_1 = groupedRecords;
    _i < groupedRecords_1.length;
    _i++
  ) {
    var record = groupedRecords_1[_i];
    var receivables = MathUtility.round(
      record.transactions
        .filter(function (t) {
          return t.debitedAccount.type === "Receivables";
        })
        .reduce(function (sum, t) {
          return sum + Number(t.grossAmount);
        }, 0),
    );
    var liabilities = record.transactions
      .filter(function (t) {
        return t.creditedAccount.type === "Liabilities";
      })
      .reduce(
        function (info, t) {
    ...
    var hasNonZeroReceivables = MathUtility.round(receivables) !== 0;
    var hasNonZeroLiability = Object.keys(liabilities).some(function (key) {
      return key !== "total" && MathUtility.round(liabilities[key]) !== 0;
    });
    if (hasNonZeroReceivables || hasNonZeroLiability) {
  ...
  var liabilitiesColumns = Object.values(vatTypesInfo)
    .filter(function (type) {
      return usedVatTypes.indexOf(type.key) !== -1;
    })
    .sort(function (a, b) {
  ...
  var rows = reportRecords.map(function (r) {
    return __spreadArray(
      ...
      liabilitiesColumns.map(function (c) {
        return r.liabilities[c.key] || 0;
      }),
      true,
    );
  });
  ...
  if (previousLineNumber && previousDatasheet) {
    var lastCol = String.fromCharCode(70 + liabilitiesColumns.length);
    var dataRangeNotation =
      previousDatasheet + "!A6:" + lastCol + previousLineNumber;
    var oldDataRange = SpreadsheetApp.getActive()
    ...
    var _loop_1 = function (oldRow) {
      ...
      if (existingRow) {
        ...
        rows.push(
          __spreadArray(
            ...
            oldRow.slice(6, 6 + liabilitiesColumns.length),
            true,
          ),
        );
      }
      ...
      for (var ci = 0; ci < liabilitiesColumns.length; ci++) {
        var oldSubVal = oldRow[6 + ci];
        if (oldSubVal) {
          var subKey = liabilitiesColumns[ci].key;
          totals.liabilities[subKey] =
            (totals.liabilities[subKey] || 0) + Number(oldSubVal);
        }
      }
    };
    for (
      var _a = 0, oldDataRange_1 = oldDataRange;
      _a < oldDataRange_1.length;
      _a++
    ) {
  ...
  rows = rows.filter(function (r) {
    return r.slice(4).some(function (v) {
      return Number(v) !== 0;
    });
  });
  var totalRow = __spreadArray(
    ...
    liabilitiesColumns.map(function (c) {
      var _a;
      return MathUtility.round(
        (_a = totals.liabilities[c.key]) !== null && _a !== void 0 ? _a : 0,
      );
    }),
    true,
  );
  ...
  datasheet
    .getRange(
      REPORT_TABLE_STARTING_ROW_NUMBER,
      1,
      1,
  ...
  if (rows.length) {
    var range = datasheet.getRange(
      REPORT_TABLE_STARTING_ROW_NUMBER + 1,
      1,
      rows.length,
      rows[0].length,
    );
    ...
    datasheet
      .getRange(
        REPORT_TABLE_STARTING_ROW_NUMBER + 1,
        NUMARIC_COLUMNS_COUNT,
        rows.length + 1,
        rows[0].length - NUMARIC_COLUMNS_COUNT,
      )
  ...
  datasheet.appendRow([
    "".concat(
      transactions.length,
      " Transactions processed. Number of records with the open balance: ",
    ) +
      "total - "
        .concat(rows.length, ", \n      reservations - ")
        .concat(
          reportRecords.filter(function (r) {
            return r.type === "Guest";
          }).length,
          ",\n      booking folios - ",
        )
        .concat(
          reportRecords.filter(function (r) {
            return r.type === "Booking";
          }).length,
          ",\n      external folios - ",
        )
        .concat(
          reportRecords.filter(function (r) {
            return r.type === "External";
          }).length,
        ),
  ]);
}
...
function getVatTypeKey(taxOrUndefined) {
  if (taxOrUndefined && taxOrUndefined.type !== "Without") {
    var type = taxOrUndefined.type,
      percent = taxOrUndefined.percent;
...
function createRecordForTransaction(transaction) {
  switch (transaction.referenceType) {
    ...
    case "Guest":
      var _a = transaction.reservation,
        id = _a.id,
        arrival = _a.arrival,
        departure = _a.departure,
        status = _a.status;
      return {
        id: id,
        type: "Guest",
    ...
    default:
      throw new Error(
        "Transactions with reference type ".concat(
...
function createHyperLinkForRecord(propertyId, r) {
  switch (r.type) {
    case "Guest":
      return '=HYPERLINK("https://app.apaleo.com/'
        .concat(propertyId, "/reservations/")
        .concat(r.id, '/folio"; "')
        .concat(r.id, '")');
    case "Booking":
      return '=HYPERLINK("https://app.apaleo.com/'
        .concat(propertyId, "/bookings/")
        .concat(r.id, '/folio"; "')
        .concat(r.id, '")');
    case "External":
      return '=HYPERLINK("https://app.apaleo.com/'
        .concat(propertyId, "/finance/folios/")
        .concat(r.id, '/general"; "')
        .concat(r.id, '")');
    default:
      return r.id;
