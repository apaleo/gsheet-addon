...
var Auth = /** @class */ (function () {
  ...
  Auth.getApaleoAuthService = function () {
    ...
    if (Auth.isApaleoApp()) {
      service
        .setScope(
          "offline_access openid profile accounting.read availability.read reports.read reservations.read identity:account-users.read setup.read",
        )
        .setCallbackFunction("Auth.authCallback");
    } else {
      service
        .setScope(
          "accounting.read availability.read reports.read reservations.read identity:account-users.read setup.read",
        )
