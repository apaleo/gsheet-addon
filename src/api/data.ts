...
//import {IdentityModels, InventoryModels, ReportsModels, FinanceModels, BookingModels, SettingsModels} from "./schema";
...
var APIData = /** @class */ (function () {
  ...
  APIData.getAccountTransactions = function (
    property,
    accountCode,
    startDate,
    endDate,
    ...
    var queryParams = [
      ...
      "from=" + DateUtility.startOf(startDate),
      "to=" + DateUtility.endOf(endDate),
    ];
    var client = Auth.getClient();
    var url = endpointUrl + "?" + queryParams.join("&");
    Logger.log(url);
  ...
  APIData.getCityTaxConfigurations = function (property) {
    var endpointUrl = apaleoApiUrl + "/settings/v1/city-tax";
    var queryParams = ["propertyId=" + property];
    var client = Auth.getClient();
    var url = endpointUrl + "?" + queryParams.join("&");
    Logger.log(url);
    var body = APIUtility.getResponseBody(client.fetch(url, defaultOptions));
    return (body && body.cityTaxes) || [];
  };
  return APIData;
})();
...
function getCurrentUserInfo() {
  ...
  var client = Auth.getClient();
  var user = APIUtility.getResponseBody(
    client.fetch("".concat(identityUrl, "/connect/userinfo"), defaultOptions),
  );
  if (!user || !user.sub) {
    throw new Error("User not found");
  }
  var detailsUrl = "".concat(identityUrl, "/api/v1/users/").concat(user.sub);
  var options = __assign(__assign({}, defaultOptions), {
    headers: {
